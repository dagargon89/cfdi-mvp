...
export function useJobs(empresaId: number, page = 1, solicitud?: 'CFDI' | 'METADATA', perPage = 50) {
  return useQuery({
    queryKey: ['jobs', empresaId, page, solicitud ?? 'todas', perPage],
    queryFn: () => api.listarJobs(empresaId, { page, solicitud, per_page: perPage }),
