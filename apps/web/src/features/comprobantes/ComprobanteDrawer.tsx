    ...
    <Drawer label="Detalle del comprobante" onClose={onClose}>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3.5">
        ...
        <dl className="m-0 flex flex-col gap-3">
          ...
          <div><dt className="text-xs text-text-muted font-semibold">Archivo XML</dt><dd className="mt-0.5 mb-0 font-mono text-xs">{comprobante.xml_path ?? '—'}</dd></div>
      ...
      <div className="border-t border-border p-4 flex gap-2">
        <Button variant="secondary" onClick={() => toast('XML descargado', 'ok')}>Descargar XML</Button>
      </div>
    </Drawer>
  );
}
