...
clave quede con `vigencia_hasta` nulo.
...
async def marcas_de_percepcion(db: AsyncSession) -> dict[str, CatalogoPercepcionMarca]:
    """Todas las marcas del §3.1 indexadas por `tipo_percepcion` — una sola consulta, para
    que un informe que recorre miles de percepciones no haga una por renglón (regla 11)."""
    filas = (await db.scalars(select(CatalogoPercepcionMarca))).all()
...
async def guardar_param_fiscal(
    ...
    *,
    ...
    manual— porque es donde viven las dos reglas que el esquema no puede sostener:

    1. **Ningún solapamiento con otro tramo de la misma clave** (ver el docstring del
       módulo). Como consecuencia directa, a lo sumo una fila por clave queda con
       `vigencia_hasta` nulo. No se cierra el tramo anterior por cuenta propia.
    ...
    tramos = list((await db.scalars(select(ParamFiscal).where(ParamFiscal.clave == clave))).all())
    for otro in tramos:
...
def _texto(bruto: object, campo: str, ctx: str, *, largo: int | None = None) -> str:
...
def _decimal(bruto: object, campo: str, ctx: str) -> Decimal:
    ...
    if isinstance(bruto, float):
        raise ErrorDeConfiguracion(
            ...
            "YAML (`valor: '117.31'`) para que se convierta exacto a Decimal."
        )
    ...
    for seccion in _SECCIONES:
        ...
        for i, fila in enumerate(_filas(documento[seccion], seccion, ruta)):
            ...
            if seccion == "param_fiscal":
                plan.params.append(_leer_param(fila, ctx))
            elif seccion == "catalogo_percepcion_marca":
                plan.marcas.append(_leer_marca(fila, ctx))
            elif seccion == "tabla_vacaciones":
                plan.vacaciones.append(_leer_vacaciones(fila, ctx))
            elif seccion == "map_departamento":
                plan.departamentos.append(
                    _FilaDepartamento(
                        departamento_texto=_texto(_requerido(fila, "departamento_texto", ctx), "departamento_texto", ctx),
                        centro_costo=_texto(_requerido(fila, "centro_costo", ctx), "centro_costo", ctx),
                    )
                )
            elif seccion == "map_concepto_provision":
                plan.provisiones.append(_leer_provision(fila, ctx))
            else:
...
def _leer_param(fila: Mapping[str, object], ctx: str) -> _FilaParam:
    clave = _texto(_requerido(fila, "clave", ctx), "clave", ctx)
    ...
    return _FilaParam(
        ...
        fuente=_texto(_requerido(fila, "fuente", ctx), "fuente", ctx),
        contexto=ctx,
    )
...
async def cargar_desde_yaml(db: AsyncSession, ruta: Path, *, empresa_id: int | None = None) -> dict[str, int]:
    """Carga un archivo de semilla y devuelve cuántos renglones procesó por tabla.
    ...
    impecable. Si al recargar cambia la cifra de un `param_fiscal`, la confirmación previa
    se limpia (ver `guardar_param_fiscal`).
    ...
    try:
        # Ordenar por (clave, vigencia_desde) permite cerrar el tramo viejo y abrir el nuevo
        # en una sola edición del archivo, sin depender de en qué orden queden los renglones.
